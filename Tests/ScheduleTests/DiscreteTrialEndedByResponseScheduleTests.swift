//
//  DiscreteTrialEndedByResponseScheduleTests.swift
//  OperantKit
//
...
final class DiscreteTrialEndedByResponseScheduleTests: XCTestCase {
    func testStructTrialResponse() {
        ...
        let consequenceObservable: Observable<Consequence> = FR(5)
            .trialResponse(5, numberOfTrials: 2)
    ...
    func testMethodChainTrialResponse() {
        let scheduler = TestScheduler(initialClock: 0)
        let observer = scheduler.createObserver(Bool.self)
        let startTime: TestTime = 0
        let completedTime: TestTime = 10000
