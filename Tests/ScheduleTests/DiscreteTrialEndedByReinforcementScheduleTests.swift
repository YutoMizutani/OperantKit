//
//  DiscreteTrialEndedByReinforcementScheduleTests.swift
//  OperantKit
//
...
final class DiscreteTrialEndedByReinforcementScheduleTests: XCTestCase {
    func testStructTrialReinforcement() {
        ...
        let consequenceObservable: Observable<Consequence> = FR(5)
            .trialReinforcement(1, numberOfTrials: 2)
    ...
    func testMethodChainTrialReinforcement() {
        let scheduler = TestScheduler(initialClock: 0)
        let observer = scheduler.createObserver(Bool.self)
        let startTime: TestTime = 0
        let completedTime: TestTime = 10000
